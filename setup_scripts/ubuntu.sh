...
if [ $(echo `pwd` | grep setup_scripts | wc -l) -ne 0 ]; then
	...
	# place spigot alongside sponsorplugin
	mkdir -p ../spigot
	cd ../spigot
	spigot_dir=`pwd`
	rm $spigot_file 2> /dev/null
	wget http://jenkins.sponsorplugin.co/job/spigot/lastSuccessfulBuild/artifact/Spigot/Spigot-Server/target/$spigot_file
	...
	echo 'Creating the plugins directory and linking the sponsorplugin plugin'
	mkdir -p plugins
	rm plugins/sponsorplugin-all.jar 2> /dev/null
	ln -s $sponsorplugin_dir/build/libs/sponsorplugin-all.jar $spigot_dir/plugins/sponsorplugin-all.jar
